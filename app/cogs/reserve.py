...
class ReserveCog(commands.Cog):
    ...
    async def character_autocomplete(
        ...
        return [
            app_commands.Choice(
                name=character.character_name[:100],
                value=character.character_name[:100],
            )
            for character in characters
            if current in character.character_name.casefold()
        ][:25]

    @app_commands.command(name="reserve", description="Reserve a character from an anime")
    ...
    @app_commands.command(name="my_character", description="Show your currently reserved character")
    async def my_character(self, interaction: discord.Interaction) -> None:
